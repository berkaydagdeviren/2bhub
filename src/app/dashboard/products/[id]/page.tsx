...
export default function ProductDetailPage() {
  ...
  const fetchSpecImages = useCallback(async () => {
  try {
    const res = await fetch("/api/products/" + productId + "/spec-images", {
      cache: "no-store",
    });
    const data = await res.json();
    if (data.images) setSpecImages(data.images);
  } catch (err) {
    console.error("Failed to fetch spec images:", err);
  }
}, [productId]);
  ...
  <div className="card p-6">
    ...
    <div className="space-y-4">
      {specImages.map((img) => (
        <div
          key={img.id}
          className="rounded-xl overflow-hidden border border-hub-border/30"
        >
          <ImageLightbox
  src={img.image_url}
...
/>
          {img.caption && (
            <div className="px-3 py-2 border-t border-hub-border/20">
              <p className="text-[11px] text-hub-secondary">{img.caption}</p>
            </div>
          )}
        </div>
      ))}
